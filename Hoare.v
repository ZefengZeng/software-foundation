...
*)
Lemma update_shadow_for_state:
  (forall {X Y: Type} {f g: X-> Y},
     (forall (x:X) ,f x = g x) -> f = g) ->
  forall (st:state) (X:id) (m n: nat),
    update (update st X n) X m = update st X m.
Proof.    
  intros functional_extensionality.
  intros.
...
Proof.
  ...
  assert (st = update st X m).
    ...
    unfold update. destruct (eq_id_dec X x0). 
    rewrite e in H0. apply H0. reflexivity.
  rewrite update_shadow_for_state.
  rewrite <- H6. apply H0.
  assumption.
...
Proof.
  ...
  exists (st X).
  apply hoare_asgn_fwd with (m:= st X) (a:=a) (P:=P) in functional_extensionality.
  unfold hoare_triple in functional_extensionality.
  apply functional_extensionality with (st).
  assumption.
  split.
  assumption.
  reflexivity.
...
Proof.
  ...
  unfold assn_sub.
  simpl.
  remember (update (update st Z (st X)) X (update st Z (st X) Y)) as st'.
  assert (H0: st' Z = st X).
  rewrite Heqst'. unfold update. simpl. reflexivity.
  assert (H1: st' X = st Y).
  rewrite Heqst'. unfold update. simpl. reflexivity.
  unfold update. simpl.
...
Proof.
  ...
  Case "True".
    eapply hoare_consequence_pre.
    eapply hoare_asgn.
    unfold bassn, assert_implies, assn_sub.
    simpl.
    intros st [_ H].
    apply le_plus_minus.
    apply ble_nat_true in H. assumption.
  Case "Fasle".
    eapply hoare_consequence_pre.
    eapply hoare_asgn.
...
Theorem hoare_if1: forall P Q b c,
  ...
  {{P}} (IF1 b THEN c FI) {{Q}}.
Proof.
  intros P Q b c H1 H2 st st' H3 H4.
  inversion H3. subst.
  apply (H1 st st'). assumption.
  split. assumption. unfold bassn. assumption.

  apply (H2 st'). apply E_Skip. 
  split. rewrite <- H6. assumption. apply bexp_eval_false. assumption.
Qed.  
(* FILL IN HERE *)
...
Proof.
  ...
  Case "True".
    eapply hoare_consequence_pre.
    eapply hoare_asgn.
    unfold bassn, assert_implies, assn_sub, update.
...
Proof. 
 ...
 Case "False".
   ...
   simpl. intros st [H1 H2].
   unfold negb in H2. destruct (st Y).
   rewrite <- H1. omega.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b1 c1,
      ...
      ceval st' (WHILE b1 DO c1 END) st'' ->
      ceval st (WHILE b1 DO c1 END) st''
  | E_Repeat_Once : forall st st' b c,
      ceval st c st' ->
      beval st' b = true ->
      ceval st (REPEAT c UNTIL b END) st'
  | E_Repeat_More : forall st st' st'' b c,
      ceval st c st' ->
      beval st' b = false ->
      ceval st' (REPEAT c UNTIL b END) st'' ->
      ceval st (REPEAT c UNTIL b END) st''.
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "E_IfTrue" | Case_aux c "E_IfFalse"
  | Case_aux c "E_WhileEnd" | Case_aux c "E_WhileLoop" 
  | Case_aux c "E_Repeat_Once" | Case_aux c "E_Repeat_More" 
(* FILL IN HERE *)
].
...
(* FILL IN HERE *)
Theorem hoare_repeat: forall Q b c,
  {{fun st => True}} c {{Q}} ->
  {{fun st => Q st /\ ~ (bassn b st)}} c {{Q}} ->
  {{fun st:state => True}} (REPEAT c UNTIL b END) {{fun st => Q st /\ (bassn b st)}}.
Proof. 
  intros.
  unfold hoare_triple.
  intros.
  remember (REPEAT c UNTIL b END) as crep.
  ceval_cases (induction H1) Case; 
    try (inversion Heqcrep); subst.
  split. apply (H st st'). assumption. assumption.
  apply bexp_eval_true. assumption.
