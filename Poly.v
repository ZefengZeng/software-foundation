...
Check length nat (cons nat 2 (nil nat)).
...
Eval compute in length nat (cons nat 1 (cons nat 2 (nil nat))).
Eval compute in length nat (cons nat 1 (nil nat)).
...
Check app nat (cons nat 2 (nil nat)) (nil nat).
Eval compute in app nat (cons nat 1 (cons nat 2 (nil nat))) (cons nat 1 (nil nat)).
...
Eval compute in (repeat true 2).
Eval compute in (repeat 1 2).
...
Check combine [1;2] [true;false].
Eval compute in (combine [1] [true]).
...
Eval compute in split ([(1,false);(2,true)]).
Example test_split:
...
Eval compute in index 2 [1;2;3;4].
...
Definition doit3times {X:Type} (f:X->X) (n:X) : X :=
  f (f (f n)).
Eval compute in doit3times (fun n => n*3) 3.
...
Eval compute in (doit3times negb false).
...
Eval compute in (filter oddb [1;2;3]).
...
Eval compute in doit3times (fun n => n*2) 2.
...
Eval compute in partition (fun x => evenb x) ([1;2;3;4;5;6]).
...
Eval compute in (fold mult [2;3;4] 2).
...
Eval compute in (fold app [[1];[];[2;3];[4]] [2]).
...
Proof. reflexivity. Qed.
Example constfun_example3 : (constfun 5) 98 = 5.
Proof. reflexivity. Qed.
...
Eval compute in (ftrue).
Eval compute in (override ftrue 1 false).
