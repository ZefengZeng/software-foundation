...
Proof.
  ...
  apply hoare_consequence_post 
    with (Q':= (fun st:state => (parity (st X)) = (parity m) /\ ~(bassn (BLe (ANum 2) (AId X)) st) )).
  ...
  intros. simpl. inversion H. rewrite <- H0. apply parity_ge_2.
  inversion H1. destruct (st X). inversion H3. destruct n. inversion H3. omega.
  ...
  intros st [Hinvar Hguard]. rewrite <- Hinvar. symmetry. apply parity_lt_2.
  unfold not. unfold not, bassn in Hguard.
  simpl in Hguard. destruct (st X).
  intros. inversion H.
  destruct n. intros. inversion H. inversion H1.
  intros. apply Hguard. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  assert (H1:(X ::= APlus (AId Y) (ANum 1)) / st || st' -> P' st -> st' X <= 5).
  apply H.
  rewrite Heqst' in H1. simpl in H1.
  unfold update in H1.
  SearchAbout eq_id_dec.
  rewrite eq_id in H1.
  apply le_S_n.
  assert (S (st Y) =  st Y + 1).
  omega.
  rewrite H2.
  apply H1.
  apply E_Ass.
...
Proof.
  ...
  intros.
  remember (update st X x) as st'.
  assert ((HAVOC X) / st || st' -> P st -> Q st').
  apply H. apply H1.
  rewrite Heqst'. apply E_Havoc.
  assumption.
Qed.
End Himp2.
(** [] *)
