...
| S n' => match m with
          | O => true
          | S m' => andb (negb (beq_nat n' m')) (ble_nat n' m')
          end
end.
...
| M n' => plus (S O) (plus (bin_to_nat n') (bin_to_nat n'))
end.

Eval compute in bin_to_nat (M BO).
Eval compute in bin_to_nat (D (M BO)).
Eval compute in bin_to_nat (D (D (M BO))).
Eval compute in bin_to_nat (M (M BO)).
Eval compute in bin_to_nat (M (D (M BO))).


Theorem plus_m_0: forall n:nat,
n + 0 = n.
Proof.
intros.
induction n.
...
reflexivity.
Qed.
Theorem plus_commute: forall n m:nat,
n + m = m + n.
Proof.
intros.
induction n.
...
reflexivity.
Qed.
Theorem plus_Sn_Sn: forall n:nat,
S n + S n = S (S (n + n)).
Proof.
intros.
induction n.
...
reflexivity.
Qed.
Theorem bin_commute: forall n:bin,
...
reflexivity.
Qed.
(*==>s (bin_to_nat n) + S (bin_to_nat n) = S (S (bin_to_nat n + bin_to_nat n))*)
