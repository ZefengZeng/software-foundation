...
Proof.
  ...
  Case "BEq a a0". simpl. rewrite optimize_0plus_sound. rewrite optimize_0plus_sound. reflexivity.
  Case "BLe a a0". simpl. rewrite optimize_0plus_sound. rewrite optimize_0plus_sound. reflexivity.
...
|E_BEq : forall (a1 a2:aexp) (n1 n2:nat),aevalR a1 n1 -> aevalR a2 n2 -> BEq a1 a2 || beq_nat n1 n2
|E_BLe : forall (a1 a2:aexp) (n1 n2:nat),aevalR a1 n1 -> aevalR a2 n2 -> BLe a1 a2 || ble_nat n1 n2
|E_BNot : forall (e:bexp) (b:bool), e || b -> BNot e || negb b
...
first;
[Case_aux c "E_BTRue" | Case_aux c "E_BFalse" | Case_aux c "E_BEq"
|Case_aux c "E_BLe" | Case_aux c "E_BNot" | Case_aux c "E_BAnd"].
...
Proof.
  ...
  generalize dependent b.
  induction e.
  SCase "E_BTrue". intros. simpl in H. rewrite <- H. apply E_BTrue.
  SCase "E_BFalse". intros. simpl in H. rewrite <- H. apply E_BFalse.
  SCase "E_BEq". intros. rewrite <- H. constructor. apply aeval_iff_aevalR. reflexivity. apply aeval_iff_aevalR. reflexivity.
  SCase "E_BLe". intros. rewrite <- H. constructor. apply aeval_iff_aevalR. reflexivity. apply aeval_iff_aevalR. reflexivity.
  SCase "E_BNot". intros. rewrite <- H. simpl. constructor. apply IHe. reflexivity.
  SCase "E_BAnd". intros. rewrite <- H. simpl. constructor. apply IHe1. reflexivity. apply IHe2. reflexivity.
Qed.
Theorem beval_iff_bevalR: forall e b,
(e || b) <-> beval e = b.
Proof.
  ...
  Case "->".
    ...
    SCase "BFalse". reflexivity.
    SCase "BEq". simpl. apply aeval_iff_aevalR in H. apply aeval_iff_aevalR in H0. rewrite H. rewrite H0. reflexivity.
    SCase "BLe". simpl. apply aeval_iff_aevalR in H. apply aeval_iff_aevalR in H0. rewrite H. rewrite H0. reflexivity.
    SCase "BNot". simpl. rewrite IHbevalR. reflexivity.
    SCase "BAnd". simpl. rewrite IHbevalR1. rewrite IHbevalR2. reflexivity.
  Case "<-".
    generalize dependent b.
    induction e.
    SCase "E_BTrue". intros. rewrite <- H. simpl. apply E_BTrue.
    SCase "E_BFalse". intros. rewrite <- H. simpl. apply E_BFalse.
    SCase "E_BEq". intros. rewrite <- H. constructor. rewrite aeval_iff_aevalR. reflexivity. rewrite aeval_iff_aevalR. reflexivity.
    SCase "E_BLe". intros. rewrite <- H. constructor. rewrite aeval_iff_aevalR. reflexivity. rewrite aeval_iff_aevalR. reflexivity.
    SCase "E_BNot". intros. simpl in H. rewrite <- H. constructor. apply IHe. reflexivity. 
    SCase "E_BAnd". intros. simpl in H. rewrite <- H. constructor. apply IHe1. reflexivity. apply IHe2. reflexivity.
Qed.
(** [] *)
End AExp.
...
Proof.
  ...
  rewrite e in H. symmetry. apply H.
  reflexivity.
Qed.
(** [] *)
...
Eval compute in (BLe (AId X) (ANum 4)).
Example bexp1 :
...
Definition XtimesYinZ_temp : com :=
Z ::= (AMult (AId X) (AId Y)).
Print XtimesYinZ.
...
|noWhileSKIP: no_whilesR (SKIP)
|noWhileAss: forall a1 a2, no_whilesR (a1 ::= a2)
|noWhileSeq: forall (a1 a2:com), no_whilesR a1 -> no_whilesR a2 -> no_whilesR (a1;;a2)
|noWhileIf: forall (b:bexp) (a1 a2:com), no_whilesR a1 -> no_whilesR a2 -> no_whilesR (IFB b THEN a1 ELSE a2 FI).
 (* FILL IN HERE *)
...
Proof.
  ...
  Case "SKIP". apply noWhileSKIP.
  Case "::=". apply noWhileAss.
  Case ";;". apply noWhileSeq. 
  apply IHc1. simpl in H. apply andb_true_elim1 in H. apply H. 
  apply IHc2. simpl in H. apply andb_true_elim2 in H. apply H.
  Case "IFB". apply noWhileIf.
  apply IHc1. simpl in H. apply andb_true_elim1 in H. apply H.
  apply IHc2. simpl in H. apply andb_true_elim2 in H. apply H.
  Case "WHILE".
  ...
  intros.
  com_cases (induction c) Case.
  Case "SKIP". simpl. reflexivity.
  Case "::=". simpl. reflexivity.
  Case ";;". simpl. inversion H. subst. apply IHc1 in H2. apply IHc2 in H3. rewrite H2. rewrite H3. simpl. reflexivity.
  Case "IFB". simpl. inversion H. subst. apply IHc1 in H2. apply IHc2 in H4. rewrite H2. rewrite H4. simpl. reflexivity.
  Case "WHILE". simpl. apply ex_falso_quodlibet. inversion H.
Qed.  
...
no_whiles c = true -> exists i, exists st', ceval_step st c i = Some st'.
Inductive no_whilesR : com->Prop :=
|noWhile
|
|
...
Eval compute in (s_execute empty_state [] [SPush 5; SPush 3; SPush 1]).
Example s_execute1 :
...
Eval compute in (s_execute (update empty_state X 3) [3;4] [SPush 4]).
Eval compute in (s_execute (update empty_state X 3) [3;4] [SPush 4;SLoad X]).
Eval compute in (s_execute (update empty_state X 3) [3;4] [SPush 4;SLoad X; SMult]).
Example s_execute2 :
...
s_execute st s1 (s_compile e ++ other) = s_execute st ((aeval st e)::s1) other.
Proof.
  induction e; try reflexivity.
  simpl in |- *.
  intros st s1 other.
  assert ((s_compile e1++ s_compile e2++[SPlus]) ++ other = s_compile e1 ++ s_compile e2 ++ [SPlus] ++ other).
  rewrite -> app_ass.
  rewrite -> app_ass.
  reflexivity.

  rewrite H in |- *.
  assert (s_execute st s1 (s_compile e1++s_compile e2++SPlus::other) = s_execute st (aeval st e1::s1) (s_compile e2 ++ SPlus::other)).
  apply IHe1.
  
  simpl in |- *.
  rewrite H0 in |- *.
  assert (s_execute st (aeval st e1::s1) (s_compile e2++SPlus::other) = s_execute st (aeval st e2 :: aeval st e1 :: s1) (SPlus::other)).
  apply IHe2.
  ...
  intros st s1 other.
  assert (s_execute st s1 (s_compile e1 ++ s_compile e2 ++ SMinus::other) = s_execute st (aeval st e1::s1) (s_compile e2 ++ SMinus::other)).
  apply IHe1.
  
  simpl in |- *.
  assert ((s_compile e1 ++ s_compile e2 ++ [SMinus])++other = s_compile e1 ++ s_compile e2 ++ [SMinus] ++ other).
  rewrite -> app_ass.
  rewrite -> app_ass.
  reflexivity.
  ...
  simpl in |- *.
  rewrite H in |- *.
  assert (s_execute st (aeval st e1 :: s1) (s_compile e2 ++ SMinus :: other) = s_execute st (aeval st e2 :: aeval st e1::s1) (SMinus:: other)).
  apply IHe2.
  ...
  simpl in |- *.
  simpl in |- *.
  assert ((s_compile e1++s_compile e2++[SMult]) ++ other = s_compile e1 ++ s_compile e2 ++ [SMult] ++ other).
  rewrite -> app_ass.
  rewrite -> app_ass.
  reflexivity.
  ...
  rewrite H in |- *.
  simpl in |- *.
  assert (s_execute st s1 (s_compile e1++s_compile e2 ++ SMult::other) = s_execute st (aeval st e1::s1) (s_compile e2 ++ SMult::other)).
  apply IHe1.
  
  rewrite H0 in |- *.
  assert (s_execute st (aeval st e1::s1) (s_compile e2 ++ SMult::other) = s_execute st (aeval st e2:: aeval st e1::s1) (SMult::other)).
  apply IHe2.
  rewrite H1 in |- *.
  simpl in |- *.
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Break: forall (st:state),
      BREAK / st || SBreak /st
  | E_Ass : forall (st:state) (X:id) (a:aexp) (n:nat),
      aeval st a = n -> (X ::= a) / st || SContinue / (update st X n)
  | E_Seq_SBreak : forall (st st':state) (c1 c2:com),
      (c1 / st || SBreak / st') -> (c1;;c2) / st || SBreak / st'
  | E_Seq_SContinue: forall (st st' st'':state) (c1 c2:com) (s:status),
      (c1 / st || SContinue / st') -> (c2 / st' || s / st'') -> (c1;;c2) / st || s / st''                                            
  | E_IfTrue : forall (b:bexp) (c1 c2:com) (st st':state) (s:status),
      beval st b = true -> c1 / st || s / st' -> (IFB b THEN c1 ELSE c2 FI) / st || s / st'
  | E_IfFlase: forall (b:bexp) (c1 c2:com) (st st':state) (s:status),
      beval st b = false -> c2 / st|| s / st' -> (IFB b THEN c1 ELSE c2 FI) / st || s / st'
  | E_WhileEnd: forall (b:bexp) (c:com) (st:state),
      beval st b = false -> (WHILE b DO c END) / st || SContinue / st
  | E_WhileLoop_SBreak: forall (b:bexp) (c:com) (st st':state),
      beval st b = true -> c / st || SBreak / st' -> (WHILE b DO c END) / st || SContinue / st'
  | E_WhileLoop_SCont: forall (b:bexp) (c:com) (st st' st'': state) (s:status),
      beval st b = true -> c / st || SContinue / st' -> (WHILE b DO c END) / st' || SContinue / st'' -> (WHILE b DO c END) / st || SContinue / st''
  (* FILL IN HERE *)
...
Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  ...
  [ Case_aux c "E_Skip" | Case_aux c "E_Break" | Case_aux c "E_Ass"
    | Case_aux c "E_Seq_SBreak" | Case_aux c "E_Seq_SContinue"
    | Case_aux c "E_IFTrue" | Case_aux c "E_IFFalse" |
    Case_aux c "E_WhileEnd" | Case_aux c "E_WhileLoop_SBreak"
    | Case_aux c "E_WhileLoop_SCon"
  (* FILL IN HERE *)
  ].
...
Proof.
  intros.
  Print ceval.
  apply E_WhileLoop_SBreak. assumption. assumption.
Qed.
...
Proof.
  ...
  ceval_cases (induction E1) Case.
  Case "E_Skip". 
  intros st2 E2. inversion E2. split. reflexivity. reflexivity.
  Case "E_Break". 
  intros st2 E2. inversion E2. split. reflexivity. reflexivity.
  Case "E_Ass". 
  intros st2 E2. inversion E2. subst. split. reflexivity. reflexivity.
  Case "E_Seq_SBreak".
  Ltac invcs H := inversion H; clear H; subst.
  intros st2 E2. invcs E2.
  apply IHE1 in H4. assumption.



  intros st2 E2. Ltac invcs H := inversion H; clear H; subst.
  invcs E2. apply IHE1. apply H4.
  destruct s2. apply IHE1 in H1. intuition idtac. inversion H0.
  generalize dependent E1. apply IHE1.
